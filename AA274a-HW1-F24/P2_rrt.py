...
class RRT(object):
    ...
    def solve(self, eps, max_iters=1000, goal_bias=0.05, shortcut=False):
        ...
        for k in range(max_iters):
            ...
            if z < goal_bias:
                ...
                x_rand = tuple(np.random.uniform(self.statespace_lo, self.statespace_hi, size=len(self.x_init)))
            x_near = tuple(V[self.find_nearest(V, x_rand), :])
            x_new = self.steer_towards(x_near, x_rand, eps)
            if self.is_free_motion(self.obstacles, x_near, x_new):
                V[n,:] = x_new
                P[n] = self.find_nearest(V, x_near) # store index in V of parent
                n += 1
                if (x_new == self.x_goal).all():
                    # Reconstruct path (P reversed)
                    success = True
                    index = P[n-1]
                    found_path = []
                    while (P[index] != -1):
                        found_path.append(V[index,:])
    ...
    def shortcut_path(self):
        ...
        while not success:
            success = True
            for x in self.path:
                if (x != self.x_init).all() and (x != self.x_goal).all():
                    x_parent = self.path[next((i for i, val in enumerate(self.path) if np.all(val == x)), -1)-1]
                    x_child =  self.path[next((i for i, val in enumerate(self.path) if np.all(val == x)), -1)+1]
                    if self.is_free_motion(self.obstacles, x_parent, x_child):
                        self.path.pop(next((i for i, val in enumerate(self.path) if np.all(val == x)), -1))
                        success = False
        ########## Code ends here ##########
...
class GeometricRRT(RRT):
    ...
    def find_nearest(self, V, x):
        ...
        _, index = tree.query(np.array(x), k=1)
        return index
        ########## Code ends here ##########
        pass
    ...
    def steer_towards(self, x1, x2, eps):
        ...
        if dist > eps:
            return tuple((1-eps/dist)*np.array(x1) + (eps/dist)*np.array(x2))
        else:
            return x2
        ########## Code ends here ##########
        pass
